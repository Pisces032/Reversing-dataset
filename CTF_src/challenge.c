...
void simple_vote_printer(struct candidate_status *cs) {
  ...
  double fraction_of_possible = (double)(cs->raw_votes) / (double)(WYOMING_POP);
...
void print_status(struct candidate_status *cs) {
  if (cs->raw_votes > WYOMING_POP) {
    printf("Candidate cannot have more votes than the population of Wyoming "
...
void main_menu() {
  int selection;
  printf("%s", "Welcome to the Wyoming Vote Tallying Software\n"
               "Presented by Jeff!\n");
  while (1) {
    selection =
